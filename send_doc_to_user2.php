...
if (isset($_POST['send_doc'])) {
    ...
    window.location.href = 'staff_folders.php';
        ...
        $reqNoStmt->close(); // Close the request number statement
    } else {
        $message = "Error preparing request number statement: " . htmlspecialchars($con->error);
    }
}

// Fetch users for sending documents, excluding the logged-in user
$userSql = "SELECT userId, fullName AS username FROM users WHERE userId != ? AND role != 'admin'";
$userStmt = $con->prepare($userSql);
$userStmt->bind_param("s", $userId); // Use $userId instead of $loggedUserId
$userStmt->execute();
