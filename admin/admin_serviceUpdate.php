...
if (isset($_POST['update'])) {
    ...
    if (strtotime($schedule) <= time()) {
        echo "<script>alert('Error: Schedule date and time must be in the future.');window.location.href = 'admin_service.php';</script>";
        exit();
    }
    ...
    $updateAcceptReqservQuery = "UPDATE acceptserv2
                                 SET schedule = ?, mpId = ?, staffId = ?  
                                 WHERE reqserv = ?";
    $stmtAcceptReqserv = $con->prepare($updateAcceptReqservQuery);
    $stmtAcceptReqserv->bind_param("siii", $schedule, $manpowerId, $staffId, $reqserv);
...
if (isset($_GET['reqserv'])) {
    ...
    // Fetch schedule from the database and add 10 days
    $scheduleOption = '';
    if (isset($row['reqserv'])) { // Assuming servId identifies the service
        $query = "SELECT schedule FROM acceptserv2 WHERE reqserv = ?";
        $stmt = $con->prepare($query);
        $stmt->bind_param('i', $row['reqserv']);
        $stmt->execute();
        $result = $stmt->get_result();

        if ($result->num_rows > 0) {
            $scheduleRow = $result->fetch_assoc();
            $scheduleDate = new DateTime($scheduleRow['schedule']);
                        ...
                        <option value="<?php echo $row['payOpt']; ?>" selected><?php echo $row['payOpt']; ?></option>
                            ...
                            <option value="Bank Transfer">Bank Transfer</option>
                                ...
                                <div class="mb-3 row">
                                    ...
                                    <div class="col">
                                        <input class="form-control" type="datetime-local" name="schedule" id="schedule" value="<?php echo htmlspecialchars(date('Y-m-d\TH:i', strtotime($row['schedule']))); ?>">
                                    </div>
                                </div>
                                <div class="mb-3 row">
                                    ...
                                    <div class="col">
                                        <select class="form-select" name="servStatus">
                                            ...
                                            <div class="div">
                                            <?php if (!empty($scheduleOption)) { ?>
                                                <label class="col col-form-label">Estimated Date:</label>
                                                <div class="col">
                                                    <input class="form-control" type="datetime-local" name="schedule" id="schedule" value="<?php echo $scheduleOption; ?>" disabled>
                                                </div>
                                                <!-- <option value="<?php echo $scheduleOption; ?>"><?php echo $scheduleOption; ?></option> -->
                                                </div>
                                            <?php } ?>
                                        </select>
                                    </div>
