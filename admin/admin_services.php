...
if (isset($_POST['deleteService'])) {
    ...
    $delete_query = "UPDATE services SET servArchive = 'Unavailable' WHERE servCode = ?";
    ...
    if ($stmt->execute()) {
        ...
        echo "<script>alert('Failed to remove service.');</script>";
        echo '<script>window.location="admin_services.php";</script>';
    }
    $stmt->close();
}
...
<body>
    <div class="container-fluid">
        <div class="col">
            <div class="row">
                <div class="col">
                    <div class="card">
                        <div class="add_service d-flex justify-content-end mt-2 me-2" style="gap: 10px;">
                            <a href="admin_addService.php">
                                <button type="button" class="btn btn-primary bg-gradient">Add Service</button>
                            </a>    
                            <a href="admin_archivedService.php">
                                <button type="button" class="btn btn-secondary bg-gradient">Archived Services</button>
                            </a>                                                                           
                        </div>
                        <div class="card-body">
                            <!-- Search Form -->
                                            ...
                                            <div class="col-lg-4 col-md-6 col-sm-12 p-2">
                                                <div class="border border-dark rounded p-3">
                                                    <div class="delete_button d-flex justify-content-end mb-3" style="gap: 10px;">
                                                        <button type="button" class="btn btn-danger bg-gradient" data-bs-toggle="modal" data-bs-target="#deleteModal" data-serv-no="<?= $service['servCode']; ?>">Remove</button> 
                                                        <a href="admin_updateService.php?servCode=<?= $service['servCode']; ?>">
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <form method="POST" action="admin_services.php" id="deleteForm">
                        <input type="hidden" name="servCode" id="servCode">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">No</button>
                        <button type="submit" name="deleteService" class="btn btn-danger">Yes</button>
                    </form>
                </div>
            </div>
        </div>
    ...
    <script type="text/javascript">
        ...
        deleteModal.addEventListener('show.bs.modal', function (event) {
            ...
            var servCode = button.getAttribute('data-serv-no'); // Extract servCode
            var inputServCode = deleteModal.querySelector('#servCode'); // Get the hidden input inside the form
            inputServCode.value = servCode; // Set the servCode value in the hidden input
        });
    </script>
</body>
</html>
