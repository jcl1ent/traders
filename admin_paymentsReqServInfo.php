...
$paymentStatus = null;
$paymentType = null;
$custId = null; // Add a variable to store custId
if ($pymntNo > 0) {
    $statusQuery = "SELECT paymentStatus, paymentType, custId FROM payment WHERE pymntNo = ?";
    ...
    if ($statusResult->num_rows > 0) {
        ...
        $custId = $row['custId']; // Retrieve custId
    }
    $statusStmt->close();
}
...
if (isset($_POST['acceptPayment'])) {
    ...
    $paymentDate = date('Y-m-d H:i:s');
        ...
        $log_action_query2 = "INSERT INTO user_action_logs (custId, action, status) VALUES (?, ?, ?)";
        $action = 'Your Payment No.' . $pymntNo . ' proof of payment has been accepted and you check now your receipt.';
        $status = 'unread';
        $log_action_stmt2 = $con->prepare($log_action_query2);
        $log_action_stmt2->bind_param("iss", $custId, $action, $status);
    ...
    echo '<script>window.location="admin_paymentsService.php"</script>';
}
?>
<!DOCTYPE html>
<html lang="en">
                        ...
                        <div class="card-header">                    
                            <div class="card-body">  
                                ...
                                <div class="back mb-3 d-flex justify-content-center" style="gap: 15px;">
                                    <a href="admin_paymentsService.php">
